...
import { ApproveRejectTimeSheetDTO } from '../Vos/ApproveRejectTimeSheetDTO';
@Component({
  selector: 'app-client-job-time-sheet-details',
  templateUrl: './client-job-time-sheet-details.component.html',
  styleUrls: ['./client-job-time-sheet-details.component.css']
})
export class ClientJobTimeSheetDetailsComponent implements OnInit, OnDestroy {
  ...
  setPaymentSummary(timesheetList): void {
    ...
    this.timesheetLength = this.timesheetList.length;
    ...
    timesheetList.forEach(timesheetData => {
      ...
      if (timesheetData.jobBidDetail.jobDetail.isPerDiem) {
        this.amountDiem = this.timesheetLength * timesheetData.jobBidDetail.clientPerDiemRate;
      }
      else {
        this.amountDiem = 0;
      }
      ...
      if (timesheetData.jobBidDetail.jobDetail.isPerDiem) {
        const timesheetPaySummaryDTO = new TimesheetPaySummaryDTO('Per Diem',
          this.timesheetLength, timesheetData.jobBidDetail.clientPerDiemRate, this.amountDiem);
        this.timesheetPaySummaryList.push(timesheetPaySummaryDTO);
      }
      else {
        const timesheetPaySummaryDTO = new TimesheetPaySummaryDTO('Per Diem',
  ...
  approveTimesheet(timesheetList, reimbursementList, amountReimbursement, totalAmount) {
    ...
    timesheetList.forEach(element => {
      ...
      if (this.overTimeHours > 0) {
        if (this.hasOvertimePayRateApplied) {
          overTimeRate = regularHourRate * 1.5;
        }
        else if (!this.hasOvertimePayRateApplied) {
          overTimeRate = regularHourRate;
        }
      }
    ...
    approveRejectTimesheetDTO.overtimeHourRate = overTimeRate;
    ...
    approveRejectTimesheetDTO.totalPerDiem = this.timesheetLength;
    approveRejectTimesheetDTO.totalReimbursementAmount = amountReimbursement;
    approveRejectTimesheetDTO.totalAmount = this.amountMilage + this.amountOverTimeHours + this.amountDiem + this.amountWorkHours + amountReimbursement;
    approveRejectTimesheetDTO.regularHourAmount = this.amountWorkHours;
    approveRejectTimesheetDTO.overtimeAmount = this.amountOverTimeHours;
    approveRejectTimesheetDTO.perDiemRateAmount = this.amountDiem;
    approveRejectTimesheetDTO.billableMilesAmount = this.amountMilage;
    approveRejectTimesheetDTO.clientId = this.localStorageService.getLoginUserId();
  ...
  calculateTotalAmount(amountReimbursement, amountDiem, amountMilage, amountWorkHours, amountOverTimeHours) {
    this.totalAmount = 0;
    this.amountMilage = amountMilage;
    this.amountDiem = amountDiem;
    this.amountOverTimeHours = amountOverTimeHours;
    this.amountWorkHours = amountWorkHours;
  ...
  getOvertimeClientDetiail(): void {
    this.approveClientService.getApproveClientDetailByClientId(this.localStorageService.getLoginUserId()).subscribe(
      data => {
