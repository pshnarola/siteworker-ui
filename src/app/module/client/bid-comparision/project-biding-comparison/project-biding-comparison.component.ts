...
export class ProjectBidingComparisonComponent implements OnInit {
  ...
  subcontractorColumns1 = [
    {
      label: this.translator.instant("sortlist"),
      value: "sortlist",
      sortable: false,
    },
    {
      label: this.translator.instant("subcontractor"),
      value: "SUB_CONTRACTOR_NAME",
      sortable: true,
    },
    {
      label: this.translator.instant("bid.amount"),
      value: "SUBCONTRACTOR_COST",
      sortable: true,
    },
    {
      label: this.translator.instant("total.experience"),
      value: "TOTAL_EXPERIENCE",
      sortable: false,
    },
    {
      label: this.translator.instant("average.rating"),
      value: "workerAvgRating",
      sortable: false,
    },
    {
      label: this.translator.instant("success.ratio"),
      value: "successRatio",
      sortable: false,
    },
  ];

  subcontractorColumns2 = [
    {
      label: this.translator.instant("sortlist"),
      value: "sortlist",
      sortable: false,
    ...
    {
      label: this.translator.instant("bid.action"),
      value: "accept",
      sortable: false,
    },
    {
  ...
  ngOnInit(): void {
    ...
    if (this._localStorageService.getItem("selectedSubcontractorForProject")) {
      this.selectedSubcontractor = this._localStorageService.getItem(
  ...
  bidAction(details, action) {
    let obj = {
      projectBidDetailId: details.id,
      bidAction: details.bidAction,
      "Return type": action,
    };
    ...
    this.projectComparisonService.bidAction(params, obj).subscribe((data) => {
      if (data.statusCode === '200' && data.message === 'OK') {
        if(action){
          this.notificationService.success(this.translator.instant('bid.accepted'), '');
        }else{
          this.notificationService.success(this.translator.instant('bid.declined'), '');
        }
    ...
    this.subscription =
      this.projectJobSelectionService.selectedProjectSubject.subscribe(
        (data) => {
          ...
          let bidDTO = this._localStorageService.getItem(
            "selectedSubcontractorForProject"
          );
          if (bidDTO !== undefined) {
            if (
              bidDTO[0].projectBidDetailDTO.projectDetail.id !==
  ...
  filterAverageRating(event): void {
    ...
    for (let i = 0; i < this.averageRatingList.length; i++) {
      ...
      if (
        averageRatingList.label.toLowerCase().indexOf(query.toLowerCase()) === 0
      ) {
  ...
  onFilterProject() {
    ...
    this.errorFlag = false;
    if (
      (this.myForm.value.minRate === null &&
        this.myForm.value.maxRate !== null) ||
    ...
    if (
      (this.myForm.value.bidSubmittedFromDate &&
        !this.myForm.value.bidSubmittedToDate) ||
      (!this.myForm.value.bidSubmittedFromDate &&
        this.myForm.value.bidSubmittedToDate)
    ) {
    ...
    if (this.myForm.value.subcontractorName) {
      this.myForm.value.subcontractorName.forEach((element) => {
        ...
        this.filterMap.set(
          "SUBCONTRACTOR_ID",
          this.filterSubcontractorFromList.toString()
        );
      });
    }
    ...
    if (this.myForm.value.bidSubmittedFromDate) {
      this.dateHelperService.setStartDate(
        this.myForm.value.bidSubmittedFromDate
      );
      const datePipe = new DatePipe("en-US");
      const value = datePipe.transform(
        this.myForm.value.bidSubmittedFromDate,
        "yyyy-MM-dd HH:mm:ss"
      );
    ...
    if (this.myForm.value.totalExperience) {
      this.myForm.value.totalExperience.forEach((element) => {
        ...
        this.filterMap.set(
          "TOTAL_EXPERIENCE",
          this.totalExperienceForFilter.toString()
        );
      });
    }
    ...
    if (!this.errorFlag) {
      ...
      this.notificationService.error(
        "Please select appropriate amount range",
        ""
      );
    }
  }
  ...
  isSubcontractorSelected(id): boolean {
    let count = 0;
    this.selectedSubcontractor.forEach(function (subcontractor) {
  ...
  onSelectSubcontractor(subcontractor): void {
    let selectedSubcontractor = this._localStorageService.getItem(
      "selectedSubcontractorForProject"
    );
  ...
  removeFromSelectedSubcontractor(subcontractor): void {
    ...
    selectedSubcontractor.forEach((element, index) => {
      if (
        subcontractor.projectBidDetailDTO.subContractor.id ===
        element.projectBidDetailDTO.subContractor.id
      ) {
  ...
  sendMessage(): boolean {
    ...
    if (this.attachmentList[3]) {
      this.notificationService.error(
        this.translator.instant("you.can.attach.maximum.three.files"),
        ""
      );
      ...
      this.chatMessageService.create(this.chatMessageDTO).subscribe((data) => {
        if (data.message === "OK" && data.statusCode === "200") {
          this.notificationService.success(
            this.translator.instant("message.sent"),
            ""
          );
          this.hideDialog();
        } else {
            ...
            if (event instanceof HttpResponse) {
              ...
              if (this.logoData.length === this.files.length) {
                this.files.forEach((element, i) => {
                  this.attachment = new ChatMessageAttachmentDTO(
                    element.name,
                    this.logoData[i]
                  );
  ...
  onRemoveFromList(id): void {
    ...
    this.files = fileTemp;
    this.notificationService.success(
      this.translator.instant("message.file.deleted"),
      ""
    );
  }
  ...
  onProjectChange() {
    if (this._localStorageService.getItem("selectedSubcontractorForProject")) {
      let data = this._localStorageService.getItem(
        "selectedSubcontractorForProject"
      );
      let selectedProject =
      ...
      if (data !== undefined && data.length !== 0) {
        if (
          data[0].projectBidDetailDTO.projectDetail.id !== selectedProject.id
        ) {
          this._localStorageService.removeItem(
            "selectedSubcontractorForProject"
          );
  ...
  loadBidDataForProject() {
    ...
    this.projectComparisonService
      .getProjectBidComparisonData(this.queryParamForProject)
      .subscribe(
        (data) => {
          if (data.statusCode === "200") {
            if (data.message === "OK") {
              ...
              this.subcontractorColumns = this.subcontractors[0]
                .projectBidDetailDTO.bidAction
                ? this.subcontractorColumns1
                : this.subcontractorColumns2;
  ...
  redirectToSubcontractor(id): void {
    CommonUtil.openWindow(
      PATH_CONSTANTS.PREVIEW_SUBCONTRACTOR_PROFILE + "?user=" + id
    );
  }
