...
@Component({
  selector: 'app-subcontractor-signup',
  templateUrl: './subcontractor-signup.component.html',
  styleUrls: ['./subcontractor-signup.component.css']
})
export class SubcontractorSignupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    element.classList.add("subcontractor-theme");
  ...
  ngOnDestroy() {
    let element = document.getElementsByTagName("body")[0];
    element.classList.remove("subcontractor-theme");
    element.classList.remove("admin-theme");
    element.classList.remove("worker-theme");
  }
    ...
    this._userService.addUser(JSON.stringify(this.myForm.value)).subscribe(
      data => {
        if (data.statusCode === '200' && data.message === 'OK') {
          this._notificationService.success(this.translator.instant('signup.successMessage.by.subcontractor'), '');
          this.submitted = false;
          setTimeout(() => {
